/////////////////////////////////////////////////////
//                                                 //
//  SCRIPT: textbox_question(sprite,subimage,text) //
// CREATOR: J-Factor                               //
//    YEAR: 2005                                   //
//                                                 //
//    DESC: Shows a dialogue textbox and returns   //
//          user input.                            //
//                                                 //
//    ARGS: arg0 = sprite                          //
//          arg1 = subimage                        //
...
var background, continue_sprite, yes_key, no_key, escape_key, speed_factor, sound, animation_speed, open;                                                          // USER VARS //

var c, cx, cy, cc, i, ii, ci, n, m, img, code, w, skip, split; // TEMP VARS //
var ix1, iy1, ix2, iy2;                                        // TEMP VARS //
var oc, oa, result;                                            // TEMP VARS //
...
x2 = room_width - 10;    // x2            :: Right x position of textbox
y2 = room_height - 10;   // y2            :: Bottom y position of textbox
...
continue_sprite = spr_continue2; // continue_sprite :: The continue sprite. When a textbox has finished showing its text
                                //                    this sprite is displayed in the bottom-right corner of the textbox.
                                //                    It can be animated.

yes_key = ord('Z');             // yes_key    :: The yes key. The key you press to:
                                //                    A. Make all the text appear instantly
                                //                    B. Answer the question in the textbox. Makes the script return true
                                
no_key = ord('X');              // no_key     :: The no key. The key you press to:
                                //                    A. Answer the question in the textbox. Makes the script return true
...
keyboard_clear(yes_key);

// INTERNAL USE ONLY //
if (is_string(argument3)) open = false;
...
while (1)
    ...
    if (keyboard_check(yes_key)) skip = 1;
...
while (1)
    ...
    if (escape_key <> -1) if (keyboard_check(escape_key)) exit;
    if ((keyboard_check(yes_key)) && (n)) {result = 1; break;}
    if ((keyboard_check(no_key)) && (n))  {result = 0; break;}
    if (!keyboard_check(yes_key)) n = 1;
...
if (split)
    return (textbox_question(sprite,subimage,string_delete(text,1,i-1)))
else
    return (result);
